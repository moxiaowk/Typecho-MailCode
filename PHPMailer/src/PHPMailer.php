...
protected function addOrEnqueueAnAddress($kind, $address, $name)
{
    ...
    if (empty($address)) {
        ...
        $this->setError($error_response);
        $this->edebug($error_response);
        $errorOutput = false; // 初始化错误输出标志为false
        return false;
    }
...
if (count($this->to) + count($this->cc) + count($this->bcc) < 1) {
    ...
    if (!$errorOutput) {
        ...
        $this->setError($json_error_response);
        $this->edebug($json_error_response);
        $errorOutput = true;
    }
    return false;
}
